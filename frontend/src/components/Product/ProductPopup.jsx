...
const ProductPopup = ({ product, isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div 
        className="bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-w-[480px] max-h-[90vh] flex flex-col relative animate-in zoom-in-95 duration-200"
        onClick={e => e.stopPropagation()}
      >
        {/* Header with Close Button */}
        <div className="w-full flex justify-end p-4 pb-0 shrink-0">
          <button 
            onClick={onClose}
            aria-label="Close"
            className="w-8 h-8 flex items-center justify-center bg-gray-100 rounded-full hover:bg-gray-200 active:scale-95 transition-all text-gray-600"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 flex flex-col gap-4 overflow-y-auto">
          <div>
            ...
            {(product.tamilName || product.nameTamil) && (
              <p className="text-sm text-green-700 font-bold mt-1">{product.tamilName || product.nameTamil}</p>
            )}
