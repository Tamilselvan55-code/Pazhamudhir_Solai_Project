...
const DeliveryHome = () => {
  ...
  const StatCard = ({ icon: Icon, label, value, colorClass, delay }) => (
    <div className={`bg-white rounded-3xl p-5 shadow-sm border border-gray-100 flex flex-col justify-between animate-in fade-in slide-in-from-bottom-4 duration-500`} style={{ animationDelay: `${delay}ms` }}>
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 pb-24">
      ...
      <div className="bg-white rounded-b-[2rem] pt-12 pb-8 px-6 shadow-sm border-b border-gray-100 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-green-50 rounded-full blur-3xl -mr-10 -mt-10 opacity-70"></div>
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            {partner?.profileImage ? (
              <img src={partner?.profileImage} alt={partner?.name} className="w-16 h-16 rounded-full object-cover shadow-md ring-4 ring-white" />
            ) : (
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center text-white font-bold text-2xl shadow-md ring-4 ring-white shrink-0">
                {partner?.name?.charAt(0).toUpperCase() || 'D'}
              </div>
            )}
            <div>
              <p className="text-sm font-semibold text-gray-500">Hello,</p>
              <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">{partner?.name?.split(' ')[0] || 'Rider'}</h1>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end">
            ...
            <span className={`text-xs font-bold mt-1.5 ${partner?.status === 'Available' ? 'text-green-600' : 'text-gray-500'}`}>
        ...
        {!partner?.isVerified && (
          <div className="mt-6 bg-orange-50 border border-orange-200 rounded-2xl p-4 flex gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-orange-800">Verification Pending</p>
              <p className="text-xs text-orange-600 mt-1">Please wait for admin approval to start receiving orders.</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-6 space-y-6">
        <h2 className="text-lg font-extrabold text-gray-900">Today's Overview</h2>
        
        {loading ? (
          <div className="grid grid-cols-2 gap-4">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="bg-white rounded-3xl p-5 shadow-sm border border-gray-100 h-32 animate-pulse flex flex-col justify-between">
                <div className="w-10 h-10 rounded-2xl bg-gray-100"></div>
                <div>
